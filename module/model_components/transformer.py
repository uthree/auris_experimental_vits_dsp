...
class RelativePositionTransformerEncoder(nn.Module):
    def __init__(
        ...
        for i in range(self.n_layers):
            self.attn_layers.append(MultiHeadAttention(hidden_channels, hidden_channels, n_heads, p_dropout=dropout, window_size=window_size))
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor):
        ...
        for i in range(self.n_layers):
            ...
            x = self.drop(x)
            x = self.norm_layers_1[i](x + res)

            res = x
            x = self.ffn_layers[i](x, x_mask)
            x = self.drop(x)
            x = self.norm_layers_2[i](x + res)
        x = x * x_mask
        return x
...
class RelativePositionTransformerDecoder(nn.Module):
    def __init__(
        ...
        self.norm_layers_2 = nn.ModuleList()
        self.ffn_layers = nn.ModuleList()
        self.norm_layers_3 = nn.ModuleList()
        for i in range(self.n_layers):
            self.self_attn_layers.append(MultiHeadAttention(hidden_channels, hidden_channels, n_heads, p_dropout=dropout, window_size=window_size))
            self.norm_layers_1.append(LayerNorm(hidden_channels))
            self.cross_attn_layers.append(MultiHeadAttention(hidden_channels, hidden_channels, n_heads, p_dropout=dropout))
            self.norm_layers_2.append(LayerNorm(hidden_channels))
            self.ffn_layers.append(FFN(hidden_channels, hidden_channels, hidden_channels_ffn, kernel_size, p_dropout=dropout))
            self.norm_layers_3.append(LayerNorm(hidden_channels))

    # x: [BatchSize, hidden_channels, Length_x]
    # x_mask: [BatchSize, 1, Length_x]
    # y: [BatchSize, hidden_channels, Length_y]
    ...
    def forward(self, x: torch.Tensor, x_mask: torch.Tensor, y: torch.Tensor, y_mask: torch.Tensor):
        ...
        for i in range(self.n_layers):
            res = x
            x = self.self_attn_layers[i](x, x, self_attn_mask)
            x = self.drop(x)
            x = self.norm_layers_1[i](x + res)

            res = x
            x = self.cross_attn_layers[i](x, y, cross_attn_mask)
            x = self.drop(x)
            x = self.norm_layers_2[i](x + res)
...
class MultiHeadAttention(nn.Module):
    ...
    def attention(self, query, key, value, mask=None):
        ...
        query = query.view(b, self.n_heads, self.k_channels, t_t).mT # [b, n_h, t_t, d_k]
        key = key.view(b, self.n_heads, self.k_channels, t_s).mT # [b, n_h, t_s, d_k]
        value = value.view(b, self.n_heads, self.k_channels, t_s).mT # [b, n_h, t_s, d_k]

        scores = torch.matmul(query / math.sqrt(self.k_channels), key.mT) # [b, n_h, t_t, t_s]
        ...
        output = output.mT.contiguous().view(b, d, t_t)  # [b, n_h, t_t, d_k] -> [b, d, t_t]
...
class FFN(nn.Module):
    def __init__(self, in_channels, out_channels, filter_channels, kernel_size, p_dropout=0.0, causal=False):
        super().__init__()
