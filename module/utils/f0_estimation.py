...
def estimate_f0_dio(wf, sample_rate=48000, segment_size=960, f0_min=20, f0_max=20000):
    if wf.ndim == 1:
        device = wf.device
...
def estimate_f0_harvest(wf, sample_rate=4800, segment_size=960, f0_min=20, f0_max=20000):
    if wf.ndim == 1:
        device = wf.device
        signal = wf.detach().cpu().numpy()
        signal = signal.astype(np.double)
